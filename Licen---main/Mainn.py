...
def start():
 ...
 best_overall = {"rmse": float('inf'), "config": None}
 
 best_per_lr = {}
 best_per_num_layers = {}
 best_per_hidden_size = {}
 best_per_input_size = {}

 for input_size in input_sizes:
    best_per_input_size[input_size] = {"rmse": float('inf'), "config": None}

    for hidden_size in hidden_sizes:
        best_per_hidden_size[(input_size, hidden_size)] = {"rmse": float('inf'), "config": None}

        for num_layers in num_layers_list:
            best_per_num_layers[(input_size, hidden_size, num_layers)] = {"rmse": float('inf'), "config": None}

            for learning_rate in learning_rates:
                best_per_lr[(input_size, hidden_size, num_layers, learning_rate)] = {"rmse": float('inf'), "epoch": None}

                best_epoch_rmse = float('inf')
                best_epoch_num = None

                for num_epochs in num_epochs_list:
                    print(f"\nTestare config: input={input_size}, hidden={hidden_size}, layers={num_layers}, lr={learning_rate}, epochs={num_epochs}")
                    ...
                    for epoch in range(num_epochs):
                         ...
                         rmse_epoch = np.sqrt(loss / len(X_train))
                         print(f"Epoca {epoch+1}/{num_epochs} - RMSE: {rmse_epoch:.6f}")
                         ...
                         if best_rmse - rmse_epoch > min_delta:
                         ...
                         if counter >= patience:
                           print(f"Early stopping la epoca {epoch+1} după {patience} epoci fără îmbunătățire.")
                           break
                    ...
                    future_predictions = predict_future(
                            model=model,
                            data=data[['AverageTemperature']].values,
                    ...
                    # Predicții pe train
                    train_seq = X_train[0].copy()
                    train_predictions = []
                    for _ in range(len(X_train)):
                        y_train_pred, _ = model.forward(train_seq)
                        y_scalar = y_train_pred[0, 0] if isinstance(y_train_pred, np.ndarray) else float(y_train_pred)
                        train_predictions.append(y_scalar)
                        train_seq = np.vstack((train_seq[1:], [[y_scalar]]))
                    ...
                    y_train_array = scaler.inverse_transform(y_train_array).flatten()

                    rmse_train = np.sqrt(np.mean((train_predictions - y_train_array) ** 2))
                    print(f"RMSE pe train: {rmse_train:.4f}")
                    ...
                    for _ in range(len(X_test)):
                        y_pred, _ = model.forward(current_seq)
                        y_scalar = y_pred[0, 0] if isinstance(y_pred, np.ndarray) else float(y_pred)
                        predictions.append(y_scalar)
                        current_seq = np.vstack((current_seq[1:], [[y_scalar]]))
                    ...
                    predictions = scaler.inverse_transform(predictions).flatten()
                    y_test_array = scaler.inverse_transform(y_test_array).flatten()

                    rmse_test = np.sqrt(np.mean((predictions - y_test_array) ** 2))  # Am schimbat rmse în rmse_test pentru claritate

                    print(f"RMSE pe test: {rmse_test:.4f}")
                    print(f"RMSE simplu (train epoch): {rmse_epoch:.4f}")

                    # Actualizări best pentru epoca curentă în cadrul acelui learning rate
                    if rmse_epoch < best_epoch_rmse:
                        best_epoch_rmse = rmse_epoch
                        best_epoch_num = num_epochs

                    # Actualizează cel mai bun rezultat pentru learning_rate fix
                    if rmse_epoch < best_per_lr[(input_size, hidden_size, num_layers, learning_rate)]["rmse"]:
                        best_per_lr[(input_size, hidden_size, num_layers, learning_rate)]["rmse"] = rmse_epoch
                        best_per_lr[(input_size, hidden_size, num_layers, learning_rate)]["epoch"] = num_epochs

                    # Actualizează best overall global
                    if rmse_epoch < best_overall["rmse"]:
                        best_overall["rmse"] = rmse_epoch
                        best_overall["config"] = {
                ...
                # Mesaj după toate epocile pentru un learning_rate fix
                print(f"\nCea mai mica eroare pentru configuratia input={input_size}, hidden={hidden_size}, layers={num_layers}, lr={learning_rate} este la epoca {best_epoch_num} cu RMSE {best_epoch_rmse:.4f}")

                # Actualizează cel mai bun rezultat pentru combinatia input, hidden, layers (pentru toate learning_rates)
                if best_epoch_rmse < best_per_num_layers[(input_size, hidden_size, num_layers)]["rmse"]:
                    best_per_num_layers[(input_size, hidden_size, num_layers)]["rmse"] = best_epoch_rmse
                    best_per_num_layers[(input_size, hidden_size, num_layers)]["config"] = {
                        "learning_rate": learning_rate,
                        "epoch": best_epoch_num
                    } 
            best_config = best_per_num_layers[(input_size, hidden_size, num_layers)]["config"]
            best_rmse = best_per_num_layers[(input_size, hidden_size, num_layers)]["rmse"]
            print(f"\nCea mai mica eroare pentru configuratia input={input_size}, hidden={hidden_size}, layers={num_layers} este cu learning rate={best_config['learning_rate']}, epoca={best_config['epoch']} si RMSE={best_rmse:.4f}")
            
            if best_config is not None:
                print(f"\nCea mai mica eroare pentru configuratia input={input_size}, hidden={hidden_size}, layers={num_layers} este cu learning rate={best_config['learning_rate']}, epoca={best_config['epoch']} si RMSE={best_rmse:.4f}")
            else:
                print(f"\n⚠️ Nicio configuratie valida pentru input={input_size}, hidden={hidden_size}, layers={num_layers}")
            
            if best_rmse < best_per_hidden_size[(input_size, hidden_size)]["rmse"]:
                best_per_hidden_size[(input_size, hidden_size)]["rmse"] = best_rmse
                best_per_hidden_size[(input_size, hidden_size)]["config"] = {
                    "num_layers": num_layers,
        ...
        # După toate num_layers pentru input, hidden
        best_config = best_per_hidden_size[(input_size, hidden_size)]["config"]
        best_rmse = best_per_hidden_size[(input_size, hidden_size)]["rmse"]
        print(f"\nCea mai mica eroare pentru configuratia input={input_size}, hidden={hidden_size} este cu layers={best_config['num_layers']}, learning rate={best_config['learning_rate']}, epoca={best_config['epoch']} si RMSE={best_rmse:.4f}")

        # Actualizează pentru input
        if best_rmse < best_per_input_size[input_size]["rmse"]:
            best_per_input_size[input_size]["rmse"] = best_rmse
            best_per_input_size[input_size]["config"] = {
                "hidden_size": hidden_size,
    ...
    # După toate hidden_sizes pentru input_size
    best_config = best_per_input_size[input_size]["config"]
    best_rmse = best_per_input_size[input_size]["rmse"]
    print(f"\nCea mai mica eroare pentru configuratia input={input_size} este cu hidden={best_config['hidden_size']}, layers={best_config['num_layers']}, learning rate={best_config['learning_rate']}, epoca={best_config['epoch']} si RMSE={best_rmse:.4f}")

 

 print("\n=== CEA MAI BUNA CONFIGURATIE GLOBALA ===")
 print(best_overall["config"])
 print(f"Cel mai mic RMSE global: {best_overall['rmse']:.4f}")
